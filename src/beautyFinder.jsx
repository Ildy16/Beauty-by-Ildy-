...
const C={
 hu:{
  ...
  next:'TOVÁBB',prev:'VISSZA',result:'MUTASD AZ ÚTVONALAMAT',restart:'ÚJRAKEZDEM',modify:'VÁLASZAIM MÓDOSÍTÁSA',stepLabel:'LÉPÉS',
  resultTitle:'A TE BEAUTY ÚTVONALAD',resultNav:{route:'ÚTVONAL',compare:'ÖSSZEHASONLÍTÁS',whyNot:'MIÉRT NEM?'},resultLead:'Nem egy „nyertes” terméket választunk. A Beauty Útvonal szerepek szerint épít rutint, és a partneri jutalék nem része a rangsorolásnak.',summaryTitle:'A VÁLASZAID RÖVIDEN',summary:{journey:'Terület',primary:'Fő cél',secondary:'További célok',skin:'Bőrprofil',age:'Korosztály',routine:'Rutin',price:'Preferencia',safety:'Figyelembe vett szempontok'},noExtra:'MOST NEM SZÜKSÉGES EXTRA LÉPÉS',noExtraText:'A válaszaid alapján nem látunk olyan kiegészítő lépést, amely érdemben javítaná a rutint.',recoveryTitle:'MOST NEM ÚJ AKTÍV A PRIORITÁS',recoveryText:'Az irritáció és a jelenlegi kezelés/aktívhasználat együtt azt jelzi, hogy most a barrier és a komfort az első. A Beauty Útvonal ezért nem épít mellé automatikusan új célzott vagy extra aktív lépést.',whyNotTitle:'MIÉRT NEM KERÜLT ELŐRÉBB?',whyNotLead:'Ezek a termékek relevánsak lehetnek a célodhoz, de a megadott biztonsági vagy tolerálhatósági szempontok miatt nem kerültek a fő ajánlások közé.',disclaimer:'A Beauty Útvonal kozmetikai és wellness tájékoztató eszköz, nem diagnózis és nem helyettesít orvosi vagy gyógyszerészi tanácsot.',
  roles:{base:'1. ALAP',targeted:'2. CÉLZOTT LÉPÉS',extra:'3. EXTRA – CSAK HA SZERETNÉD'},
  why:'MIÉRT EZT?',details:'RÉSZLETES ADATLAP',alternatives:'TOVÁBBI RELEVÁNS LEHETŐSÉGEK',compareTitle:'GYORS ÖSSZEHASONLÍTÁS',compareLead:'Legfeljebb három releváns lehetőség egymás mellett – nem rangsor, hanem áttekintés.',compareLabels:{goal:'Fő fókusz',ingredients:'Kulcsösszetevők',routine:'Rutinszint',status:'Finder státusz'},wellnessRoutine:'WELLNESS',
  ...
  deviceTitle:'Beauty készülékek — előbb technológia, aztán modell',deviceText:'Készülékeknél nem adunk automatikus modellajánlást, mert az ellenjavallatok és a biztonság konkrét eszközönként eltérhetnek. A Beauty Tech útmutatóban összehasonlíthatod az RF, LED, mikroáram és más technológiákat, valamint azt is, mire érdemes figyelni választás előtt.',deviceCta:'BEAUTY TECH ÚTMUTATÓ',wellnessTitle:'Wellness — biztonságosan, bizonyítékokkal',wellnessText:'Wellness és étrend-kiegészítők esetén nem adunk automatikus termékajánlást. A Wellness tudástárban a bizonyítékokat, korlátokat, kölcsönhatási és biztonsági szempontokat külön értelmezzük.',wellnessCta:'WELLNESS ÚTMUTATÓ',
 ...
 en:{
  back:'Back to home',eyebrow:'BEAUTY BY ILDY · PERSONAL BEAUTY JOURNEY',title:'Find what fits you',
  lead:'A few short questions help us organise the skincare, haircare, wellness and beauty-tech options most relevant to you.',
  start:'START BEAUTY FINDER',note:'About 1–2 minutes · free · no sign-up',privacy:'Answers stay in your browser in this version; they are not placed in the URL or used to create a health profile.',
  howTitle:'How it works',howSteps:['Choose where you want help.','Tell us your main goal and a few important preferences.','The Finder builds a personalised route from those answers.'],
  ...
  next:'NEXT',prev:'BACK',result:'SHOW MY ROUTE',restart:'START AGAIN',modify:'EDIT MY ANSWERS',stepLabel:'STEP',
  resultTitle:'YOUR BEAUTY ROUTE',resultNav:{route:'ROUTE',compare:'COMPARE',whyNot:'WHY NOT?'},resultLead:'We do not pick a single “winner”. The Finder builds roles into a routine, and partner commission is not part of ranking.',summaryTitle:'YOUR ANSWERS AT A GLANCE',summary:{journey:'Area',primary:'Main goal',secondary:'Other goals',skin:'Skin profile',age:'Age group',routine:'Routine',price:'Preference',safety:'Factors considered'},noExtra:'NO EXTRA STEP NEEDED RIGHT NOW',noExtraText:'Based on your answers, we do not see an additional step that meaningfully improves the routine.',recoveryTitle:'A NEW ACTIVE IS NOT THE PRIORITY RIGHT NOW',recoveryText:'Active irritation together with current treatment or active use makes barrier support and comfort the priority. The Finder therefore does not automatically add a new targeted or extra active step.',whyNotTitle:'WHY WASN’T THIS PRIORITISED?',whyNotLead:'These products may relate to your goal, but your safety or tolerance answers kept them out of the main recommendations.',disclaimer:'Beauty Finder is a cosmetic and wellness information tool, not a diagnosis and not a substitute for medical or pharmacist advice.',
  roles:{base:'1. BASE',targeted:'2. TARGETED STEP',extra:'3. EXTRA – ONLY IF YOU WANT IT'},
  why:'WHY THIS?',details:'DETAILED PROFILE',alternatives:'OTHER RELEVANT OPTIONS',compareTitle:'QUICK COMPARISON',compareLead:'Up to three relevant options side by side – an overview, not a ranking.',compareLabels:{goal:'Main focus',ingredients:'Key ingredients',routine:'Routine level',status:'Finder status'},wellnessRoutine:'WELLNESS',
  ...
  deviceTitle:'Beauty devices — technology first, model second',deviceText:'We do not generate automatic device-model recommendations because contraindications and safety considerations can vary by device. Use the Beauty Tech guide to compare RF, LED, microcurrent and other technologies, and to see what matters before choosing a device.',deviceCta:'BEAUTY TECH GUIDE',wellnessTitle:'Wellness — evidence and safety first',wellnessText:'For wellness and supplements, we do not generate automatic product recommendations. The Wellness knowledge area separates evidence, limitations, interaction risks and safety considerations.',wellnessCta:'WELLNESS GUIDE',
 ...
 de:{
  back:'Zurück zur Startseite',eyebrow:'BEAUTY BY ILDY · PERSÖNLICHER BEAUTY-WEG',title:'Finde, was zu dir passt',
  lead:'Einige kurze Fragen helfen dabei, die für dich relevanten Optionen aus Hautpflege, Haarpflege, Wellness und Beauty Tech übersichtlich zu ordnen.',
  start:'BEAUTY FINDER STARTEN',note:'Ca. 1–2 Minuten · kostenlos · ohne Anmeldung',privacy:'Die Antworten bleiben in dieser Version nur im Browser; sie werden nicht in die URL geschrieben und nicht zu einem Gesundheitsprofil zusammengeführt.',
  howTitle:'So funktioniert es',howSteps:['Wähle, wobei du Unterstützung möchtest.','Nenne dein Hauptziel und einige wichtige Präferenzen.','Der Finder erstellt daraus deinen persönlichen Weg.'],
  ...
  deviceTitle:'Beauty-Geräte — zuerst die Technologie, dann das Modell',deviceText:'Wir geben keine automatischen Geräte-Modellempfehlungen, weil Kontraindikationen und Sicherheitsaspekte je nach Gerät unterschiedlich sein können. Im Beauty-Tech-Guide kannst du RF, LED, Mikrostrom und weitere Technologien vergleichen und sehen, worauf du vor der Auswahl achten solltest.',deviceCta:'BEAUTY-TECH-GUIDE',wellnessTitle:'Wellness — Evidenz und Sicherheit zuerst',wellnessText:'Bei Wellness und Nahrungsergänzungsmitteln geben wir keine automatischen Produktempfehlungen. Im Wellness-Wissensbereich trennen wir Evidenz, Grenzen, mögliche Wechselwirkungen und Sicherheitsaspekte.',wellnessCta:'WELLNESS-GUIDE',
...
function WhyNot({items,t}){
 ...
 return <section className="finderWhyNot" id="finder-why-not"><h2>{t.whyNotTitle}</h2><p>{t.whyNotLead}</p><div className="finderWhyNotGrid">{items.map(item=><article key={item.product.slug}><small>{item.product.brand}</small><h3>{item.product.name}</h3><p>{item.reasons.map(x=>t.whyNotReasons?.[x]).filter(Boolean).join(' · ')}</p><a href={'#product-'+item.product.slug}>{t.details}<ArrowRight size={14}/></a></article>)}</div></section>
}

function FinderCompare({results,t,lang}){
 const items=[...(results?.primary||[]),...(results?.alternatives||[])].slice(0,3);
 if(items.length<2)return null;
 return <section className="finderCompare" id="finder-compare"><h2>{t.compareTitle}</h2><p>{t.compareLead}</p><div className="finderCompareGrid">{items.map(item=><article key={item.product.slug}>
  <small>{item.product.brand}</small><h3>{item.product.name}</h3>
  <dl>
   <div><dt>{t.compareLabels.goal}</dt><dd>{item.meta.goals.slice(0,2).map(g=>goalLabels[lang]?.[g]||g).join(' · ')}</dd></div>
   <div><dt>{t.compareLabels.ingredients}</dt><dd>{item.product.ingredients.slice(0,3).map(x=>localIngredient(x,lang)).join(' · ')}</dd></div>
   <div><dt>{t.compareLabels.routine}</dt><dd>{item.meta.routineLevels.map(x=>x==='wellness'?t.wellnessRoutine:(t.routineOptions?.[x]?.[0]||x)).join(' · ')}</dd></div>
   <div><dt>{t.compareLabels.status}</dt><dd>{t.warnings[item.status]}</dd></div>
  </dl>
  <a href={'#product-'+item.product.slug}>{t.details}<ArrowRight size={14}/></a>
 </article>)}</div></section>
}
...
export function BeautyFinder({lang='hu'}){
 ...
 useEffect(()=>{if(started)window.scrollTo({top:0,behavior:'smooth'})},[step,showResult,started]);
 const restart=()=>{setStarted(false);setStep(0);setShowResult(false);setA({journey:null,primaryGoal:null,secondaryGoals:[],skinType:null,ageBand:null,routineLevel:'balanced',pricePreference:'best-match',currentRetinoid:false,sensitive:false,irritated:false,prescription:false,pregnancy:false,sunscreen:false,adult:null,medication:false,wellnessPregnancy:false,caffeineSensitive:false,hormonalConcern:false,eyeSensitive:false,multipleAcids:false})};

 if(!started)return <main className="finderPage finderLanding"><section className="finderHero"><div className="finderHeroInner"><div className="finderHeroCopy"><a href="#top" className="finderBack"><ArrowLeft size={14}/>{t.back}</a><p className="eyebrow">{t.eyebrow}</p><h1>{t.title}</h1><p className="finderHeroLead">{t.lead}</p><div className="finderHeroActions"><button className="finderPrimary" onClick={()=>setStarted(true)}>{t.start}<ArrowRight size={16}/></button><small>{t.note}</small></div></div><aside className="finderIntroCard"><p className="finderIntroTitle">{t.howTitle}</p><ol>{t.howSteps.map((item,i)=><li key={item}><span>{String(i+1).padStart(2,'0')}</span><p>{item}</p></li>)}</ol><div className="finderPrivacy"><ShieldCheck size={18}/><span>{t.privacy}</span></div></aside></div></section></main>;
 ...
 if(showResult){
  ...
  return <main className="finderPage">
   ...
   <section className="finderResults" id="finder-route" aria-live="polite">
    ...
    <FinderCompare results={results} t={t} lang={lang}/>
