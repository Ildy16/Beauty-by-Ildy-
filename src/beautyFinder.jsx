...
const C={
 hu:{
  ...
  why:'MIÉRT EZT?',details:'RÉSZLETES ADATLAP',alternatives:'TOVÁBBI RELEVÁNS LEHETŐSÉGEK',compareTitle:'GYORS ÖSSZEHASONLÍTÁS',compareLead:'Legfeljebb három releváns lehetőség egymás mellett – nem rangsor, hanem áttekintés.',compareLabels:{goal:'Fő fókusz',ingredients:'Kulcsösszetevők',routine:'Rutinszint',status:'Finder státusz'},
 ...
 en:{
  ...
  why:'WHY THIS?',details:'DETAILED PROFILE',alternatives:'OTHER RELEVANT OPTIONS',compareTitle:'QUICK COMPARISON',compareLead:'Up to three relevant options side by side – an overview, not a ranking.',compareLabels:{goal:'Main focus',ingredients:'Key ingredients',routine:'Routine level',status:'Finder status'},
 ...
 de:{
  ...
  why:'WARUM DIESES?',details:'DETAILPROFIL',alternatives:'WEITERE PASSENDE OPTIONEN',compareTitle:'SCHNELLVERGLEICH',compareLead:'Bis zu drei relevante Optionen nebeneinander – als Überblick, nicht als Rangliste.',compareLabels:{goal:'Hauptfokus',ingredients:'Schlüsselwirkstoffe',routine:'Routine',status:'Finder-Status'},
...
function FinderCompare({results,t,lang}){
 ...
 return <section className="finderCompare" id="finder-compare"><h2>{t.compareTitle}</h2><p>{t.compareLead}</p><div className="finderCompareGrid">{items.map(item=><article key={item.product.slug}>
  ...
  <dl>
   ...
   <div><dt>{t.compareLabels.routine}</dt><dd>{item.meta.routineLevels.map(x=>t.routineOptions?.[x]?.[0]||x).join(' · ')}</dd></div>
...
export function BeautyFinder({lang='hu'}){
 ...
 return <main className="finderPage"><section className="finderFlow">
  ...
  {step===5&&a.journey==='skin'&&<><h1>{t.age}</h1><p className="finderSub">{t.ageSub}</p><div className="finderChoiceGrid">{Object.entries(t.ageBands).map(([k,v])=><Toggle key={k} active={a.ageBand===k} onClick={()=>patch({ageBand:k})}>{v}</Toggle>)}</div></>}
